# Boogu-Image-0.1-Turbo full-weight RL (FlowGRPO), vllm_omni rollout, OCR reward
#
# FULL-WEIGHT ONLY (no LoRA): the Boogu rollout adapter is a self-contained
# vllm-omni pipeline built from plain nn.Linear layers, so LoRA weight sync
# from the trainer is not supported — keep lora_rank=0.
#
# The Turbo checkpoint is a 4-step DMD student: num_inference_steps stays 4
# for both training and validation, and CFG is forbidden (no negative prompt).
#
# Prerequisite: preprocess the OCR dataset for Boogu:
#   python examples/flowgrpo_trainer/data_process/boogu_ocr.py \
#       --input_dir ~/data/ocr \
#       --output_dir ~/data/ocr/boogu_image
set -x
...
ROLLOUT_TP=1
REWARD_TP=4
# Must be divisible by 16 (VAE stride 8 x patch size 2).
...
python3 -m verl_omni.trainer.main_diffusion \
    ...
    trainer.experiment_name=boogu_image_ocr \
