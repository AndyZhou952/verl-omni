...
"""
Preprocess the OCR dataset to parquet format (for Boogu-Image-Turbo training).
You can obtain the raw dataset from https://github.com/yifan123/flow_grpo/tree/main/dataset/ocr

The chat-message format reproduces the Boogu T2I encoding exactly
(BooguImagePipeline.SYSTEM_PROMPT_4_T2I_UNIFIED + user instruction under the
checkpoint's Qwen3VL chat template). The Turbo (DMD) student forbids
classifier-free guidance, so no negative prompt is emitted.
"""
...
# Must byte-match verl_omni.pipelines.boogu_image_flow_grpo.common.BOOGU_SYSTEM_PROMPT_T2I
# (kept literal here so the preprocessor has no verl_omni import dependency).
