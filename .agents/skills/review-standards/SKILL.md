...
description: "verl-omni's maintainer review standard: the priority order every PR is judged by, the approval gate, scope/split rules, and the house communication style for review comments. Distilled from the full review history of this repository (747 inline + 334 conversation review comments across 151 reviewed PRs). Use when reviewing a PR, predicting what maintainer review will demand, or writing review comments in the house style."
---

# verl-omni Review Standards

The maintainer review stance in this repository is **empiricist gatekeeping**: RL training
code fails silently, so proof beats reasoning ("we cannot trust AI with 100% confidence, a
physical evidence is still essential", #128). Gating works by **withholding approval**, not
by formal change-requests (1 CHANGES_REQUESTED in 619 review submissions): comments iterate
in rounds and the PR merges only on a terse "LGTM". Adopt that model: every finding is
either a reason not to approve yet, or not worth posting.

The deepest pattern, visible in every module: **interrogative minimalism against upstream
verl as the spec**. The modal review comment is a short "why do we need this?" whose
default answer is "delete it, or use verl's design" — and a concrete technical
justification flips the verdict to "ok" immediately. Extensibility here comes from correct
placement and registries with a minimal surface — never from speculative hooks ("I prefer
not to add the interface unless it is really necessary").
...
- [ ] No monkey-patching of verl/vllm-omni/diffusers; any unavoidable patch is minimal and
      carries an owner-attributed TODO with removal condition and reference link
      ("# TODO (<owner>): drop this patch in vllm-omni next version upgrade. Ref: <link>", #94).
...
significant PRs, ping a second maintainer for confirmation.

## Scope / split rules

- >~20 files or multiple concerns bundled → require a split, and **propose the concrete
  cut**: "consider to split into 3 sub-prs for easier review :) 1. General Interface PR
  2. Qwen-Image Editing PR 3. A document PR after PR1/PR2 merge?" (#238). Justification:
  "clear in git log and easy to debug & review" (#153).
...
- Deferral is fine via owner-attributed TODO in code ("leave todo to drop later, otherwise
  I will forget", #165) — the TODO must name an owner and a removal trigger.

## Communication style (writing review comments in the house style)
...
- **On pushback**: if the author's evidence holds, concede immediately — "ok", "agree",
  ...
  Admit mistakes plainly ("If it is, I am sorry about this.", #283).
...
- **Delegate by area**: "@<owner> PTAL" with a one-clause reason. Weight-sync/zmq changes
  go to the owning rollout expert; NPU/Docker changes go to the Ascend owners; verify
  claimed fixes actually landed before resolving a thread ("Reverted." → "Not yet.").

## What NOT to comment on (noise to avoid)
...
- Perfection demands on experimental/recipe code — accept with an owner-attributed TODO or
  "fine for this time".
- CPU-test coverage gaps when GPU/e2e tests cover the behavior (#263).
- Generic praise or diff recaps — the longest review messages are reserved for evidence
  disputes and architecture, never summaries.
