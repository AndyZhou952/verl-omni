...
# boogu/models/transformers/rope.py, commit a366095.
# Modified: repo ruff auto-fixes only (typing modernization, zip strict=False); no functional changes.
"""
...
class BooguImageRotaryPosEmbed(nn.Module):
    ...
    def get_freqs_cis(
        ...
        for i, (d, e) in enumerate(zip(axes_dim, axes_lens, strict=False)):
    ...
    def forward(
        ...
        seq_lengths = [
            ...
            for cap_len, ref_img_len, img_len in zip(
                l_effective_cap_len, l_effective_ref_img_len, l_effective_img_len, strict=False
            )
        ]
        ...
        for i, (cap_seq_len, seq_len) in enumerate(zip(l_effective_cap_len, seq_lengths, strict=False)):
            ...
            if ref_img_sizes[i] is not None:
                for ref_img_size, ref_img_len in zip(ref_img_sizes[i], l_effective_ref_img_len[i], strict=False):
                    H, W = ref_img_size
...
class BooguImageDoubleStreamRotaryPosEmbed(nn.Module):
    ...
    def forward(
        ...
        combined_img_seq_lengths = [
            ...
            for ref_img_len, img_len in zip(l_effective_ref_img_len, l_effective_img_len, strict=False)
        ]
