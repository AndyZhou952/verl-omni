...
# boogu/taylorseer_utils/__init__.py, commit a366095.
...
def _tree_sub(lhs, rhs):
    if isinstance(lhs, tuple):
        return tuple(_tree_sub(x, y) for x, y in zip(lhs, rhs, strict=False))
    return lhs - rhs
...
def _tree_add(lhs, rhs):
    if lhs is None:
        return rhs
    if isinstance(lhs, tuple):
        return tuple(_tree_add(x, y) for x, y in zip(lhs, rhs, strict=False))
    return lhs + rhs
