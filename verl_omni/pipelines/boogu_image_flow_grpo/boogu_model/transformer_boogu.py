...
# boogu/models/transformers/transformer_boogu.py, commit a366095.
# Modified: rewrote intra-package imports as relative imports within boogu_model.
# Modified: removed os.environ["device"] gating; Triton RMSNorm replaced by torch.nn.RMSNorm.
# Modified: attention processor selection now keyed on is_flash_attn_available() and
#           torch.cuda.is_available() instead of the "device" env var.
# Modified: repo ruff auto-fixes (typing modernization, zip strict=False, removed unused
#           locals, wrapped a long f-string); no functional changes.
"""
...
class BooguImageTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if is_flash_attn_available() and torch.cuda.is_available():
            try:
...
class BooguImageDoubleStreamTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if self.modulation:
            ...
            for i, (encoder_seq_len, seq_len) in enumerate(zip(encoder_seq_lengths, seq_lengths, strict=False)):
...
class BooguImageTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        instruction_feature_configs: dict[str, Any] = dict(  # noqa: B006
            instruction_feat_dim=1024,
            reduce_type="mean",
            num_instruction_feat_layers=1,
        ),
        prompt_tuning_configs: dict[str, Any] = dict(use_prompt_tuning=False),  # noqa: B006
    ...
    def img_patch_embed_and_refine(
        ...
        max_combined_img_len = max(
            [
                ...
                for img_len, ref_img_len in zip(l_effective_img_len, l_effective_ref_img_len, strict=False)
            ]
        )
        ...
        for i, (ref_img_len, img_len) in enumerate(zip(l_effective_ref_img_len, l_effective_img_len, strict=False)):
    ...
    def preprocess_instruction_hidden_states(
        ...
        if isinstance(raw_instruction_hidden_states, torch.Tensor):
            ...
            raise ValueError(
                f"Invalid type of raw_instruction_hidden_states, expected torch.Tensor or list, "
    ...
    def forward(
        ...
        for i, (img_size, img_len, seq_len) in enumerate(
            zip(img_sizes, l_effective_img_len, seq_lengths, strict=False)
        ):
