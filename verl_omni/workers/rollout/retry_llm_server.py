...
class WholeSampleRetryLLMServerClient(LLMServerClient):
    """LLM server client that retries the whole sample on abort.

    Used where a partial generation cannot be resumed: diffusion rollouts
    (no mid-denoise resume) and AR rollouts under whole-sample recovery.

    Behavior:
    - Call the base ``LLMServerClient.generate`` with the original (unmodified)
      ``prompt_ids`` and ``sampling_params`` every attempt.
    - If the returned output's ``stop_reason`` is ``"aborted"`` / ``"abort"``,
      sleep briefly and retry the whole sample.
    ...
    async def generate(
        ...
        **kwargs: Any,
        ...
        """Generate a sample, retrying the whole sample on abort."""
        ...
        for attempt in range(self.max_retries):
            ...
            logger.debug(
                "rollout aborted (attempt %d/%d) for request_id=%s; retrying whole sample",
                attempt + 1,
                self.max_retries,
                request_id,
            )
