#!/usr/bin/env bash
# Qwen3-Omni Thinker GSPO + LoRA fully-async V1 smoke test (minimal runtime).
#
# Exercises trainer_mode=omni_fully_async: 1 GPU for the FSDP trainer plus 1 GPU
# for a standalone TP=1 rollout replica, NCCL checkpoint engine, staleness gate
# k=1, LoRA adapter-only weight sync (lora.merge=False). Follows
# run_gspo_qwen3_omni_thinker_lora_v1_smoke.sh with fully-async overrides.
#
# Requires: verl, verl-omni, vllm-omni installed; 2 GPUs.
#
# Override via env: NUM_TRAIN_GPUS, NUM_ROLLOUT_GPUS, MODEL_PATH, DATA_DIR, TOTAL_TRAIN_STEPS
set -xeuo pipefail
...
# ── Run fully-async V1 training (3 steps, LoRA, GSPO, standalone rollout) ─────
python3 -m verl_omni.trainer.main_omni \
    ...
    actor_rollout_ref.rollout.n_gpus_per_node="${NUM_ROLLOUT_GPUS}" \
    actor_rollout_ref.rollout.tensor_model_parallel_size="${NUM_ROLLOUT_GPUS}" \
    ...
    trainer.v1.omni_fully_async.staleness_threshold=1 \
    trainer.v1.omni_fully_async.parameter_sync_step=1 \
    trainer.v1.omni_fully_async.rollout_recovery=continue \
    trainer.val_before_train=false \
    trainer.balance_batch=True \
    trainer.critic_warmup=0 \
    trainer.logger=console \
    trainer.project_name=verl-test \
    trainer.experiment_name=gspo-qwen3-omni-thinker-lora-fully-async-e2e-v1 \
...
echo "Qwen3-Omni Thinker GSPO+LoRA fully-async e2e V1 smoke test passed (training completed successfully)."
